...
class _ClassificationScreenState extends State<ClassificationScreen> {
            ...
            GridView.builder(
              ...
              itemBuilder: (ctx, i) {
                ...
                return GestureDetector(
                  ...
                  child: Container(
                    decoration: BoxDecoration(color: isSelected ? Colors.green : Colors.grey[200], borderRadius: BorderRadius.circular(15), border: Border.all(color: isSelected ? Colors.green : Colors.grey)),
                    child: Center(child: Text(t, style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: isSelected ? Colors.white : Colors.black87))),
                  ),
                );
              },
            ),
  ...
  Widget _buildDetailedDropdown(String label, Map<String, String> dataMap, Function(String?) onChanged) {
    return Padding(
      ...
      child: DropdownButtonFormField<String>(
        ...
        items: dataMap.entries.map((e) => DropdownMenuItem(value: e.key, child: Text("${e.key} - ${e.value}"))).toList(),
        onChanged: onChanged,
      ),
    );
  }
  ...
  void _navigateToDetailedForm() {
    ...
    Navigator.push(context, MaterialPageRoute(builder: (context) => FormScreen(observation: newObs)));
  }
}